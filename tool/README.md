...
```bash
API_URL=https://api.kmstry.net SITE_URL=https://staging.kmstry.net \
BUILD_NAME=1.0.1 BUILD_NUMBER=43 RELEASE_CHECKS_ONLY=true \
bash tool/release_gate.sh
```

Replace hosts and version/build values for the intended environment. There is
no implicit release environment. Omit `RELEASE_CHECKS_ONLY` to build the Android
